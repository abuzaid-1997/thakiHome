    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            GeometryReader { geometry in
                ...
                // ✅ 1. أضفنا VStack هنا ليحمل الهيدر وتحته المحتوى
                VStack(spacing: 0) {
                    
                    // ✅ 2. أضفنا الهيدر هنا في القمة
                    HomeHeaderView()
                    
                    // ✅ 3. محتواك القديم كما هو (داخل ZStack)
                    ZStack(alignment: .bottom) {
                        ...
                        // التوغل بار السفلي (Dashboard | All Devices)
                        VStack {
                            if isEditing {
                    ...
                    .frame(width: width)
                    // (تنبيه: قم بنقل الـ frame max height infinity إذا لزم الأمر، لكن غالباً سيعمل تلقائياً)
                }
                
            }
            
            .background(Color.themeBackground.ignoresSafeArea()) // الخلفية لكامل الشاشة
        ...
        .onChange(of: currentHomeId) { oldValue, newValue in
